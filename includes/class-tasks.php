...
class Tasks {
	...
	private function add_meta( $hook, $value ) {
		...
		$insert = $wpdb->insert(
			"{$wpdb->prefix}quillsmtp_task_meta",
			array(
				'hook'         => $hook,
	...
	private function update_meta( $id, $data, $data_format = '%s' ) {
		...
		return (bool) $wpdb->update( "{$wpdb->prefix}quillsmtp_task_meta", $data, array( 'ID' => $id ), $data_format, array( '%d' ) );
	}
	...
	private function get_meta( $id ) {
		...
		$meta = $wpdb->get_row(
			$wpdb->prepare(
				"
					SELECT *
					FROM {$wpdb->prefix}quillsmtp_task_meta
					WHERE ID = %d
				",
				$id
			),
