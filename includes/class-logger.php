...
use QuillSMTP\Interfaces\Log_Handler_Interface;
...
class Logger implements Logger_Interface {
	 ...
	 *
	 * @param array  $handlers Optional. Array of log handlers. If $handlers is not provided, the filter 'quillsmtp_register_log_handlers' will be used to define the handlers. If $handlers is provided, the filter will not be applied and the handlers will be used directly.
	...
	public function __construct( $handlers = null, $threshold = null ) {
		if ( null === $handlers ) {
			$handlers = apply_filters( 'quillsmtp_register_log_handlers', array() );
		}
	...
	public function add( $handle, $message, $level = Log_Levels::NOTICE ) {
		$message = apply_filters( 'quillsmtp_logger_add_message', $message, $handle );
		$this->log(
			$level,
			$message,
			array(
	...
	public function log( $level, $message, $context = array() ) {
		if ( ! Log_Levels::is_valid_level( $level ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( '<code>%1$s</code> was called with an invalid level "%2$s".', 'Logger::log', esc_html( $level ) ), '1.0.0' );
		}

		if ( $this->should_handle( $level ) ) {
			$timestamp = time();
			$message   = apply_filters( 'quillsmtp_logger_log_message', $message, $level, $context );
	...
	public function clear_expired_logs() {
		$days      = absint( apply_filters( 'quillsmtp_logger_days_to_retain_logs', 30 ) );
